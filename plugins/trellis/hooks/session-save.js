#!/usr/bin/env node
// Trellis Session Save - Stop hook
//
// Updates .trellis/STATE.md "Last:" timestamp when a session ends.
// Keeps STATE.md accurate for session continuity — the next session
// can see when work last happened via /trellis:status.
//
// Design: intentionally minimal. Just update the timestamp line.
// Complex state saving is handled by the trellis commands themselves.
...
process.stdin.on('end', () => {
  ...
  try {
    ...
    const statePath = path.join(cwd, '.trellis', 'STATE.md');
