#!/usr/bin/env node
// Trellis Audit Nudge - PostToolUse hook
//
// Counts git commits and nudges (or auto-triggers) when audit thresholds
// are reached. Reads config from .trellis/trellis.yaml stewardship section.
//
// Behavior:
//   - Only activates on Bash tool calls that contain "git commit" (not --amend)
//   - Increments per-lens commit counters in .trellis/.audit-tracker.json
...
process.stdin.on('end', () => {
  ...
  try {
    ...
    const yamlPath = path.join(trellisDir, 'trellis.yaml');
    if (!fs.existsSync(yamlPath)) process.exit(0);

    // Read trellis.yaml for stewardship config (simple YAML parse)
    const yamlContent = fs.readFileSync(yamlPath, 'utf8');
    const audits = parseAuditsConfig(yamlContent);
    if (!audits) process.exit(0);

    // Read or create tracker
    const trackerPath = path.join(trellisDir, '.audit-tracker.json');
    let tracker = {
      commits: {
    ...
    for (const [lens, config] of Object.entries(audits)) {
      ...
      if (count >= threshold || earlyTrigger) {
        if (mode === 'auto') {
          messages.push(
            `TRELLIS AUDIT AUTO-TRIGGER: ${lens.charAt(0).toUpperCase() + lens.slice(1)} audit threshold reached ` +
            `(${count} commits since last ${lens} audit). Run /trellis:audit ${lens} ` +
          ...
          messages.push(
            `TRELLIS AUDIT NUDGE: It's been ${count} commits since your last ${lens} audit. ` +
            `Consider running /trellis:audit ${lens} when you have a moment. ` +
            `(Configure thresholds in .trellis/trellis.yaml under stewardship.audits)`
          );
        }
      }
    }
    ...
    if (messages.length > 0) {
      ...
      const nudgePath = path.join(tmpDir, `trellis-audit-${sessionId}-nudged`);
