#!/usr/bin/env node
// Waddle Context Monitor - PostToolUse hook
//
// Reads context metrics from the statusline bridge file and injects
// waddle-specific warnings when context usage is high.
//
...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Only activate when waddle is initialized in this project
    if (!fs.existsSync(path.join(cwd, '.waddle', 'STATE.md'))) {
      process.exit(0);
    }
    ...
    // Debounce
    const warnPath = path.join(tmpDir, `waddle-ctx-${sessionId}-warned.json`);
    ...
    if (isCritical) {
      message =
        `WADDLE CONTEXT CRITICAL: Usage at ${usedPct}%. Remaining: ${remaining}%. ` +
        'Context is nearly exhausted. Finish the current commit if possible, then inform ' +
        'the user that context is low. Progress is tracked in .waddle/STATE.md — ' +
        'they can run /waddle:status in a fresh session to continue.';
    } else {
      message =
        `WADDLE CONTEXT WARNING: Usage at ${usedPct}%. Remaining: ${remaining}%. ` +
        'Context is getting limited. Avoid starting new complex work. Wrap up the ' +
        'current task and commit. Progress is saved in .waddle/STATE.md.';
    }
