...
The user's argument follows `/waddle:audit`. Determine mode:
...
1. Read the audit lenses reference: Glob `**/waddle/references/audit-lenses.md`, then read it.
2. Read the conventions reference: Glob `**/waddle/references/conventions.md`, then read it.
...
1. Read `.waddle/.audit-tracker.json` if it exists — check last audit date for this lens
...
- Read VISION.md (path from waddle.yaml `stewardship.vision`) if the lens requires it and the file exists
- Read DECISIONS.md (path from waddle.yaml `stewardship.decisions`) if the lens requires it and the file exists
- Read ARCHITECTURE.md (path from waddle.yaml `stewardship.architecture`) if configured and the lens requires it
...
- For ARCHITECTURE.md specifically: check if a template exists (Glob `**/waddle/templates/ARCHITECTURE.md.template`).
  ...
  - options:
    - label: "Create"
      description: "Scaffold ARCHITECTURE.md from the waddle template"
    - label: "Skip"
      description: "Run the audit without it"
  If Create: read the template, replace {{PROJECT_NAME}} (from waddle.yaml), write to the configured path.
...
- For other documents: note as an info-level finding:
  "<Lens> audit would benefit from <Document>. Consider creating one via /waddle:bootstrap or manually."
...
prompt: |
  You are a waddle AUDIT worker performing a <lens> audit.
  ...
  ## Output
  - Save your audit report to .waddle/audits/YYYY-MM-DD-<lens>.md
  ...
  - Log any codebase discoveries to .waddle/STATE.md Learnings section
  - Do NOT write to .waddle/BACKLOG.md — the orchestrator handles backlog updates after you finish
model: <from waddle.yaml models.worker, default sonnet>
```
...
1. Create `.waddle/audits/` directory if it doesn't exist (`mkdir -p .waddle/audits`)
2. For lightweight mode: save a combined report to `.waddle/audits/YYYY-MM-DD-sweep.md`
3. Update `.waddle/.audit-tracker.json`:
...
1. Read `.waddle/BACKLOG.md` (create from template if missing: Glob `**/waddle/templates/BACKLOG.md.template`)
2. For each **critical** and **warning** finding:
   ...
   - Append new findings under the matching severity heading in the `## Open` section:
     ...
     ```
3. Add significant findings as learnings in `.waddle/STATE.md` (patterns worth remembering)
...
Show findings with the looking penguin:

```
   _
  (o)   Audit Complete: <lens(es)>
  /V\
  (_)>  <N> findings (<critical> critical, <warning> warning, <info> info)
  ~~
```

List critical findings first, then warnings. Info items can be summarized.

If critical findings exist, suggest `/waddle:do` tasks to address them:
"Consider running `/waddle:do fix <issue description>` for critical findings."
