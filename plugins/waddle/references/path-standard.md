...
4. Assign next plan number:
   - Check if `.waddle/plans/` exists, create with `mkdir -p` if needed
   - Glob `.waddle/plans/*.md` to find highest NNN
   - Use NNN+1 (first plan is 001)
5. Write plan to `.waddle/plans/NNN-<slug>.md` with `status: draft`
...
8. Find and read the conventions reference: Glob `**/waddle/references/conventions.md`, then read it. You will paste relevant sections from this file into each worker's spawn prompt.
9. Read `.waddle/waddle.yaml` for specialist config and worker model
10. Determine if a specialist applies:
    - Check specialists config in waddle.yaml
...
prompt: |
  You are a waddle IMPLEMENT worker.
  ...
  Then review their code against plan criteria, commit with waddle protocol, and log learnings.
  <if no specialist>:
  Implement directly.
  ...
  - Log learnings and pending decisions to .waddle/STATE.md
model: <from waddle.yaml models.implementer if set, else models.worker, default sonnet>
```
...
prompt: |
  You are a waddle REVIEW worker.
  ...
  <"Learning Protocol" section>
model: <from waddle.yaml models.reviewer if set, else models.worker, default sonnet>
```

17. Show looking penguin with review summary:
```
   _
  (o)   Review complete!
  /V\
...
18. Process review result — extract verdict from the review output:
    - Find `<waddle:verdict>...</waddle:verdict>` in the output and extract the inner text (PASS or FIXME)
...
prompt: |
  ...
  - One commit per fix.
model: <from waddle.yaml models.fixer if set, else models.worker, default sonnet>
```
...
22. Update `.waddle/STATE.md` Current section:
    - Focus: idle
    - Plan: completed plan path
    - Progress: done
    - Last: current timestamp
23. Show happy penguin:
```
   _
  (v)   All done! Plan #NNN complete.
 //-\\
 (\_/)
  ^ ^
```
