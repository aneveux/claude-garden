# Waddle Worker Protocol

> **Documentation only** — workers are not invoked via subagent_type. They are spawned by `/waddle:do` with inline prompts that inject selected sections from `conventions.md`. This file documents the worker protocol for reference and maintenance.
...
**REVIEW**: Run the four-pass escalating review (spec compliance -> functional -> challenge -> adversarial). Output structured review report with `<waddle:verdict>PASS</waddle:verdict>` or `<waddle:verdict>FIXME</waddle:verdict>`, FIXME items, NOTES, and LEARNINGS. Do NOT modify code - only report findings.

**PLAN**: Research the codebase, understand the scope, and write a plan file to `.waddle/plans/`. Follow the plan format specification provided in your task prompt. Report the file path as `<waddle:plan_path>path</waddle:plan_path>`.
